...
interface CreateActionItemModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (item: Omit<ActionItem, 'id' | 'total_hours'>) => void;
  initialData?: Partial<ActionItem>;
}

const CreateActionItemModal: React.FC<CreateActionItemModalProps> = ({ isOpen, onClose, onSave, initialData }) => {
  const getInitialState = (): Omit<ActionItem, 'id'> => {
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Create Action Item">
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <input type="text" name="subject" id="subject" value={formData.subject} onChange={handleChange} className="mt-1 block w-full rounded-md border-neutral-300 shadow-sm focus:border-primary-blue focus:ring-primary-blue sm:text-sm" required />
        </div>
        <div>
